...
public class BalloonSpawner : MonoBehaviour {
	...
	private enum BalloonKind { SLOW, NORMAL, SPEED, JUMP, ARBITRARY };
	private enum BalloonSpawnThreshold { SLOW = 0, NORMAL = 30, SPEED = 75, JUMP = 95 };
	...
	private void spawnBalloon(float location, BalloonKind balloonKind) {
		...
		if(balloonKind == BalloonKind.ARBITRARY){
			...
			if (randomNumber > (int)BalloonSpawnThreshold.JUMP) { balloonKind = BalloonKind.JUMP;} 
			else if (randomNumber > (int)BalloonSpawnThreshold.SPEED) { balloonKind = BalloonKind.SPEED;} 
			else if (randomNumber > (int)BalloonSpawnThreshold.NORMAL) { balloonKind = BalloonKind.NORMAL;} 
			else { balloonKind = BalloonKind.SLOW;} 
		}
	...
	private Vector3 getSpawnPosition (float yLocation) {
		...
		if (balloonSectionIterator >= 2) { balloonSectionIterator = 0;}
